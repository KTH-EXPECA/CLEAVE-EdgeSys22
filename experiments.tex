...
In this section, we demonstrate the utility of \ac{CLEAVE} by walking readers through an example use-case for the tool.
With this, we aim to answer questions relating to the ability of our framework to provide accurate and repeatable measurements of the performance of \acp{NCS} deployed on edge computing infrastructure.
...
Moreover, there are a number of video-streaming applications running concurrently on this Edge setup.
...
\ac{CLEAVE} is deployed on a testbed consisting of \num{10} Raspberry Pi 4B  clients connected wirelessly to an IEEE 802.11n \ac{AP}; connected to the Ethernet backbone of this \ac{AP} is a general-purpose \verb|x86_64| desktop computer acting as a Cloudlet/Edge server.
...
We deploy varying numbers of control loops inside this Swarm, executing plant containers exclusively in the Raspberry Pi clients and controller containers in the Cloudlet.
...
The first step in aswering the questions posed above is to evaluate the baseline performance of the control system, both locally and over the network.
We first configure \ac{CLEAVE} to run a single loop with both plant and controller co-located on the Edge server; next we configure an identical loop to be executed with the plant located on a Raspberry Pi client and the controller on the Edge server.
For each setup, we execute a series of scenarios varying:
...
After the initial \num{10} runs, we identify that setups with sampling rates of \SIlist{5;10}{\hertz} were consistently too unstable to consider, and discard them from further analyses.
For all other setups, we further note that setups with no additional delay are always stable and thus decided to also discard them from further analyses.
Remaining setups, i.e.\ sampling rates \( \geq \) \SI{20}{\hertz} and artificial delays \( \geq \) \SI{25}{\milli\second}, are then repeated an additional \num{20} times for better statistical significance.
...
\begin{figure}[t]
    ...
    \begin{subfigure}[t]{\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{single_loop_toppled}
        \caption{Fraction of toppled plants.}\label{fig:single:topple}
    \end{subfigure}\\
    \begin{subfigure}[t]{\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{single_loop_rms}
        \caption{Mean angle \acs*{RMS}; red line indicates \( y = 180 \).}\label{fig:single:rms}
    \end{subfigure}\\
    \begin{subfigure}[t]{\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{single_loop_rtts}
        \caption{
            Mean \acsp*{RTT}.
...
Now that we have established baselines for single loops in the use case scenario, we can proceed to studying the interaction between the \acp{NCS} and the video-streaming applications.
...
Based on the baseline results obtained beforehand, we execute setups with \ac{NCS} plant sampling rates of \SIlist{20;40;60}{\hertz}.
...
\begin{figure}[t]
    ...
    \begin{subfigure}[h]{.22\textwidth}
        \centering
        \includegraphics[width=\textwidth]{video_topple_frac}
        \caption{Toppled plants.}\label{fig:video:toppled}
    \end{subfigure}%
    \hfill%
    \begin{subfigure}[h]{.22\textwidth}
        \centering
        \includegraphics[width=\textwidth]{video_angle_rms}
        \caption{Angle \ac{RMS}.}\label{fig:video:rms}
    \end{subfigure}\\
    \begin{subfigure}[h]{.22\textwidth}
        \centering
        \includegraphics[width=\textwidth]{video_drop_frac}
        \caption{Packet losses.}\label{fig:video:drop}
    \end{subfigure}%
    \hfill%
    \begin{subfigure}[h]{.22\textwidth}
        \centering
        \includegraphics[width=\textwidth]{video_rtt}
...
\cref{fig:video:toppled} shows the fraction of repetitions of each scenario in which \emph{at least} one plant failed to maintain stability and toppled.
\cref{fig:video:rms} shows the \ac{RMS} for the pendulum angles for each scenario, only considering data from plants that did not topple.
\cref{fig:video:drop} shows the fraction of \ac{UDP} datagrams dropped, averaged over all plants and repetitions per scenario.
\cref{fig:video:rtt} shows the measured end-to-end plant-side \ac{RTT}, averaged over all plants and repetitions per scenario.
