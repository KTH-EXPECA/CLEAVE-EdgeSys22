...

{
    ...
    \begin{itemize}
        \item Inverted pendulum is broadly used as a benchmark in NCS (already mentioned); worth mentioning related work which does the same thing!
        \item Easy to re-parameterize for different system dynamics.
        \item Partly a reality, similar to deployments in industry right now (although I'm pretty sure industry uses ZigBee or WirelessHART).
        \item WiFi is a bit of a problem - best case would've been to have 4G or 5G. Argument could however be that 4G at least has higher RTTS, so if it doesn't work for WiFi it will definitely not work for 4G\dots
        \item Video streaming/analytics are core use-case for edge computing (already in text).
    \end{itemize}
}

\todo[inline]{
    More strongly argue why this is an interesting and general use-case scenario.
    Paper needs to argue and describe why this use case chosen is one of general interest, addressing key aspects of demands, and should refer to a comparative view with related work.
}
