...
% make caption and subcaption work with ieetran
\makeatletter
...
\todo[inline,caption={}]{
  \begin{enumerate}
    \item Do a haircut, i.e., eliminate the lines that take less than half a line at the ending of a paragraph.
    \item Your captions for a lot of figures are really long, try to shorten them significantly.
    \item Reduce the reference list to 15 - 20 references.
    \item I don't think that resorting to hardware-in-the-loop is the right way to go here. There is for sure a definition somewhere and maybe it fits. But spirit-wise this is not the right association we want to generate with a reviewer/reader. What we are really doing is virtualized CPS emulation. Maybe that is a better way to go. Also, in the abstract etc. we are working out too little that CLEAVE is really just a platform with an API, on which any type of real CPS could be added. I think you should work that out in the abstract etc as well.
  \end{enumerate}
}
